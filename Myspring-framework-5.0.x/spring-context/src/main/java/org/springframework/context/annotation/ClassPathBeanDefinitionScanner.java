...
/**
   ...
   registering corresponding(相应的 合适的) bean definitions with a given registry ({@code BeanFactory}
 ...
 *
   ...
   一个bean定义扫描器，它检测类路径上的bean候选对象，使用给定的注册表({@code BeanFactory})注册相应的bean定义 或{@code ApplicationContext})

   通过可配置的类型筛选器检测候选类
   默认过滤器包括用Spring注释的类
   {@link org.springframework.stereotype。组件@ Component},
   {@link org.springframework.stereotype。库@ Repository},
   {@link org.springframework.stereotype。服务@ Service},
   或 {@link org.springframework.stereotype。控制器@ Controller}刻板印象
 */
