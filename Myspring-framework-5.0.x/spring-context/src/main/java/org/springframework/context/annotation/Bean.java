 ...
 *
   ...
   指示方法生成由Spring容器管理的bean。
   此注释的属性的名称和语义是有意为之的 类似于Spring XML模式中的{@code <bean/>}元素
   虽然有一个{@link #name}属性，但是默认的策略是 确定bean的名称是使用{@code @Bean}方法的名称。
   这是方便和直观的，但如果需要显式命名，则可以使用{@code name}属性(或其别名{@code value})。
 */
