...
/**
 ...
 这是bean容器的基本客户端视图; 进一步的接口，如{@link ListableBeanFactory}和 {@link org.springframework.beans.factory.config.ConfigurableBeanFactory} 是否有特定用途

 这个接口是由包含许多bean定义的对象实现的， 每个字符串名都是唯一标识的。根据bean的定义， 工厂将返回一个包含对象的独立实例
 (原型(原形)设计模式),或一个共享实例(一个优越 可选的单例设计模式，其中实例为 工厂范围内的单例)。
 将返回哪种类型的实例 取决于bean工厂配置:API是相同的。因为spring2.0，因为spring2.0，根据具体的应用程序，可以使用更多的作用域上下文(如。“请求”和“会话”在web环境中适用)。
